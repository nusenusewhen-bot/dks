...
class HardwareMouse {
  ...
  async moveTo(targetX, targetY, options = {}) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      await this.page.mouse.move(baseX + tremorX + arcOffset + overshootX, baseY + tremorY + (arcOffset * 0.3) + overshootY);
  ...
  async click(element) {
    ...
    await this.moveTo(targetX + Math.cos(approachAngle) * approachDist, targetY + Math.sin(approachAngle) * approachDist, { targetSize: 120 });
    ...
    for (let i = 0; i < microAdjustments; i++) {
      await this.moveTo(targetX + (Math.random() - 0.5) * 10, targetY + (Math.random() - 0.5) * 10, { targetSize: 30 });
      await new Promise(r => setTimeout(r, 50 + Math.random() * 100));
    }
    
    await this.moveTo(targetX, targetY, { targetSize: 15 });
    await new Promise(r => setTimeout(r, 60 +(x * 2.1) * 0.25 + Math.sin(x * 4.3) * 0.125;
  }
  ...
  async click(element) {
    ...
    if (Math.random() > 0.3) {
      await this.page.mouse.move(this.x + (Math.random() - 0.5) * 15, this.y + (Math.random() - 0.5) * 15);
    }
...
class SessionBehavior {
  ...
  async randomHover() {
    ...
    if (Math.random() > 0.7) {
      await this.page.mouse.move(box.x + box.width/2 + (Math.random() - 0.5) * 30, box.y + box.height/2 + (Math.random() - 0.5) * 20);
  ...
  async simulateReading() {
    ...
    while (Date.now() < endTime) {
      ...
      if (action < 0.3) {
        await this.page.mouse.move(this.mouse.x + (Math.random() - 0.5) * 100, this.mouse.y + (Math.random() - 0.5) * 50);
      } else if (action < 0.5) {
  ...
  async simulateDistraction() {
    ...
    if (distractionType < 0.4) {
      ...
      await this.page.mouse.move(this.mouse.x + (Math.random() - 0.5) * 200, this.mouse.y + (Math.random() - 0.5) * 150);
      await new Promise(r => setTimeout(r,(x * 2.1) * 0.25 + Math.sin(x * 4.3) * 0.125;
  }
...
class CaptchaSolver {
  ...
  async detectType(frame) {
    const prompt = await frame.evaluate(() => {
      const selectors = ['.prompt-text', '.challenge-text', '.hcaptcha-challenge-text', '.task-description', 'h2', 'h3', '.text'];
    ...
    if (lower.includes('andere') || lower.includes('different') || lower.includes('anders') || lower.includes('odd one')) {
  ...
  async solveVisualDifferent(frame) {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      await this.mouse.moveTo(box.x + box.width/2, box.y + box.height/2);
      await new Promise(r =>(x * 2.1) * 0.25 + Math.sin(x * 4.3) * 0.125;
  }
...
class CaptchaSolver {
  ...
  computeAnswer(question) {
    ...
    const contradictions = [
      ['carp', 'karpfen', 'fish', 'animal'],
      ['sugar', 'zucker', 'food'],
      ['carnivore', 'fleischfresser', 'animal'],
      ['drug', 'droge', 'substance']
    ];
    
    const foundCategories = [];
    contradictions.forEach(([...patterns, category]) => {
    ...
    if (lower.includes('can') || lower.includes('darf') || lower.includes('may') || lower.includes('eat') || lower.includes('fressen')) {
      const negative = lower.includes('not') || lower.includes('kein') || lower.includes('nicht') || lower.includes('without') || lower.includes('ohne');
      return negative ? (lower.includes('ja') ? 'nein' : 'no') : (lower.includes('ja') ? 'ja' : 'yes');
    }
    
    if (lower.includes('is') || lower.includes('ist') || lower.includes('are')) {
      if (foundCategories.length >= 2 && uniqueCategories.length > 1) {
  ...
  async solveCheckbox(frame) {
    const checkbox = await frame.$('#checkbox.   async solveCheckbox(frame) {
...
module.exports = { HardwareMouse, SessionBehavior, CaptchaSolver };
