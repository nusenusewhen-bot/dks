...
class SessionBehavior {
  ...
  async simulateDistraction() {
    ...
    if (distractionType < 0.4) {
      ...
      await this.naturalScroll(-1,< 0.98) {
        overshootX = (Math.random() - 0.5) * this.profile.overshoot * 1.5;
        overshootY = (Math.random() - 0.5) * this.profile.overshoot * 1.5;
      }
...
class VisualAnalyzer {
  async analyzeElement(element) {
    try {
      ...
      const objects = this.segmentObjects(imageData.data, img.width, img.height);
...
class CaptchaSolver {
  ...
  async solveVisualDifferent(frame) {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        const analysis = await this.visualAnalyzer.analyzeElement(images[i]);
        imageData.push({ index: i, element: images[i], box: box, analysis: analysis });
      } catch (e) {
  ...
  async solveTextChallenge(frame) {
    const question = await frame.evaluate(() => {
      const el = document.querySelector('.prompt-text, .challenge-text, h< 0.98) {
        overshootX = (Math.random() - 0.5) * this.profile.overshoot * 1.5;
        overshootY = (Math.random() - 0.5) * this.profile.overshoot * 1.5;
      }
...
class CaptchaSolver {
  ...
  computeAnswer(question) {
    ...
    if (detected.length >= 2) {
      ...
      if (unique.length > 1) {
        if (detected.some(d => d.category === 'animal') && detected.some(d => ['food', 'substance', 'mineral'].includes(d.category))) {
          return lower.includes('ja') || lower.includes('nein') ? 'nein' : 'no';
        }
      }
    }
    
    if (lower.includes('can') || lower.includes('darf') || lower.includes('may') || lower.includes('allowed') || lower.includes('eat') || lower.includes('fressen')) {
